...
- `icons`: small markers for resources, mechanics, roles, phases and specific actions; values may be short text or local asset references such as `icon:banana`, resolved under `assets/icons`. Use plain keys for resources/families (`money`, `exchange`) and scoped keys for table entities (`role:captain`, `phase:attack-run`, `action:fire-torpedo`). The bundled icon set is credited in `assets/ATTRIBUTION.md`;
...
Local sound references such as `sound:coup-declared` are served from `assets/sounds/coup-declared.wav` and shown as preview buttons in the dashboard/player theme panels. Bundled event sounds are credited in `assets/ATTRIBUTION.md`; current cues are original synthesized jingles and effects, not generic UI clicks.
